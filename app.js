...
function openRecentDb() {
  if (!("indexedDB" in window)) {
    return Promise.reject(new Error("This browser cannot save MP3 history."));
  }

  return new Promise((resolve, reject) => {
    const request = indexedDB.open("pulse-music-library", 1);
    request.onupgradeneeded = () => {
      const db = request.result;
      const store = db.createObjectStore("recentTracks", { keyPath: "id" });
...
function dbTransaction(mode = "readonly") {
  return recentDb.transaction("recentTracks", mode).objectStore("recentTracks");
}
...
async function getRecentTracks() {
  ...
  const tracks = await requestToPromise(dbTransaction().getAll());
...
async function renderRecentTracks() {
  ...
  if (!tracks.length) {
    els.recentList.innerHTML = "<p>No saved MP3s yet.</p>";
    return;
  }

  els.recentList.innerHTML = tracks.map((track) => `
    <button class="recent-item" type="button" data-recent-id="${encodeURIComponent(track.id)}">
...
async function saveRecentFiles(files) {
  ...
  await Promise.all(files.map((file, index) => requestToPromise(store.put({
    id: trackId(file),
    name: file.name,
  ...
  const tracks = await getRecentTracks();
  await Promise.all(tracks.slice(10).map((track) => requestToPromise(dbTransaction("readwrite").delete(track.id))));
  await renderRecentTracks();
}

async function loadRecentTrack(id) {
  const track = await requestToPromise(dbTransaction().get(id));
  if (!track) {
    return;
  }
  ...
  loadFilesIntoPlaylist([file]);
  els.uploadStatus.textContent = `Loaded ${track.name}`;
}

async function clearRecentTracks() {
  if (!recentDb) {
    return;
  }
...
els.fileInput.addEventListener("change", () => {
  ...
  saveRecentFiles(files).catch(() => {
    els.uploadStatus.textContent = "Loaded, but browser storage could not save these MP3s.";
  });
});
...
els.recentList.addEventListener("click", (event) => {
  ...
  loadRecentTrack(decodeURIComponent(item.dataset.recentId)).catch(() => {
    els.uploadStatus.textContent = "Could not load that saved MP3.";
  });
});

els.clearRecentButton.addEventListener("click", () => {
  clearRecentTracks().catch(() => {
    els.uploadStatus.textContent = "Could not clear saved MP3s.";
  });
});
...
openRecentDb()
  .then((db) => {
    recentDb = db;
    return renderRecentTracks();
  })
  .catch(() => {
    els.recentList.innerHTML = "<p>Recent MP3 saving is unavailable in this browser.</p>";
  });
